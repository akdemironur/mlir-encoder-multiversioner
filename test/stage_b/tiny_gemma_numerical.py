#!/usr/bin/env python3
"""Reference numerical checks for the Stage B tiny encoder dispatch."""
...
def make_input(sequence_length: int) -> np.ndarray:
    rng = np.random.default_rng(0xC0DE + sequence_length)
    return normal(rng, (1, sequence_length, HIDDEN), 0.1)


def encoder_body(x: np.ndarray, parameters: Params) -> np.ndarray:
    ...
    attn = (q + k + v) @ o_w
    residual = attn + x_2d
    norm = residual * norm_scale + norm_bias
    ff1_biased = norm @ ff_w1 + ff_b1
    ff1 = ff1_biased * ff1_biased
    ff2 = ff1 @ ff_w2
    out = ff2 + ff_b2 + residual
    return out.reshape(1, sequence_length, HIDDEN).astype(np.float32)


def encoder_generic(x: np.ndarray, parameters: Params) -> np.ndarray:
    return encoder_body(x, parameters)
...
def encoder_static(
    ...
    if x.shape[1] != sequence_length:
        raise AssertionError(
            f"@encoder_s{sequence_length} got S={x.shape[1]}, "
            f"expected S={sequence_length}"
        )
    return encoder_body(x, parameters)


def encoder(x: np.ndarray, parameters: Params) -> tuple[str, np.ndarray]:
    parameter_ids = tuple(id(parameter) for parameter in parameters)
    sequence_length = x.shape[1]
    if sequence_length in STATIC_LENGTHS:
        result = encoder_static(x, parameters, sequence_length)
        selected = f"@encoder_s{sequence_length}"
    else:
        result = encoder_generic(x, parameters)
        selected = "@encoder_generic"

    if tuple(id(parameter) for parameter in parameters) != parameter_ids:
        raise AssertionError("@encoder did not preserve parameter references")
...
def main() -> int:
    ...
    for sequence_length in STATIC_LENGTHS:
        ...
        selected, wrapper_y = encoder(x, parameters)
        assert_path(selected, f"@encoder_s{sequence_length}", sequence_length)
        assert_close(
            f"S={sequence_length} wrapper vs static",
            wrapper_y,
            encoder_static(x, parameters, sequence_length),
        )
        assert_close(
            f"S={sequence_length} static vs generic",
            wrapper_y,
            encoder_generic(x, parameters),
        )

    for sequence_length in FALLBACK_LENGTHS:
        x = make_input(sequence_length)
        selected, wrapper_y = encoder(x, parameters)
        assert_path(selected, "@encoder_generic", sequence_length)
        assert_close(
    ...
    print(
        "PASS Stage B tiny encoder numerics: "
