module {
  func.func @encoder_block(
    ...
    %norm0_empty = tensor.empty(%seq) : tensor<?x64xf32>
    %norm0 = linalg.generic {
      indexing_maps = [
        affine_map<(s, h) -> (s, h)>,
        affine_map<(s, h) -> (h)>,
        affine_map<(s, h) -> (h)>,
    ...
    ins(%x_2d, %norm_scale, %norm_bias
        : tensor<?x64xf32>, tensor<64xf32>, tensor<64xf32>)
    outs(%norm0_empty : tensor<?x64xf32>) {
    ...
    %q_init = linalg.fill ins(%zero : f32) outs(%q_empty : tensor<?x64xf32>)
        -> tensor<?x64xf32>
    %q = linalg.matmul
        ins(%norm0, %q_w : tensor<?x64xf32>, tensor<64x64xf32>)
    ...
    %k_init = linalg.fill ins(%zero : f32) outs(%k_empty : tensor<?x64xf32>)
        -> tensor<?x64xf32>
    %k = linalg.matmul
        ins(%norm0, %k_w : tensor<?x64xf32>, tensor<64x64xf32>)
    ...
    %v_init = linalg.fill ins(%zero : f32) outs(%v_empty : tensor<?x64xf32>)
        -> tensor<?x64xf32>
    %v = linalg.matmul
        ins(%norm0, %v_w : tensor<?x64xf32>, tensor<64x64xf32>)
    ...
    %residual0_empty = tensor.empty(%seq) : tensor<?x64xf32>
    %residual0 = linalg.generic {
      indexing_maps = [
        affine_map<(s, h) -> (s, h)>,
        affine_map<(s, h) -> (s, h)>,
        affine_map<(s, h) -> (s, h)>
    ...
    outs(%residual0_empty : tensor<?x64xf32>) {
    ^bb0(%value: f32, %skip: f32, %unused: f32):
      %out = arith.addf %value, %skip : f32
      linalg.yield %out : f32
    } -> tensor<?x64xf32>

    %norm1_empty = tensor.empty(%seq) : tensor<?x64xf32>
    %norm1 = linalg.generic {
      indexing_maps = [
        affine_map<(s, h) -> (s, h)>,
        affine_map<(s, h) -> (h)>,
        affine_map<(s, h) -> (h)>,
    ...
    ins(%residual0, %norm_scale, %norm_bias
        : tensor<?x64xf32>, tensor<64xf32>, tensor<64xf32>)
    outs(%norm1_empty : tensor<?x64xf32>) {
    ^bb0(%value: f32, %scale: f32, %bias: f32, %unused: f32):
      %scaled = arith.mulf %value, %scale : f32
      %out = arith.addf %scaled, %bias : f32
      linalg.yield %out : f32
    ...
    %ff1_mm = linalg.matmul
        ins(%norm1, %ff_w1 : tensor<?x64xf32>, tensor<64x256xf32>)
